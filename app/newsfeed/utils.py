...
from .models import NewsFeed, Subscription
...
def get_posts_for_user(user_id, limit=10):
    ...
    posts_prefetch = Prefetch('posts', queryset=Post.objects.filter(blog__id__in=subscriptions).order_by('-created_at')[:limit])
